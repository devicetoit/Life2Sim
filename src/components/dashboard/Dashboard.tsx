...
export const Dashboard: React.FC<DashboardProps> = ({
    ...
    const assets = useStore((state) => state.data.assets);
    const policyEnabled = useStore((state) => state.data.settings.policy?.enabled !== false);
    const deathAge = useStore((state) => state.data.settings.deathAge || 0);
    ...
    return (
        <div className="h-screen bg-slate-50 flex flex-col font-sans selection:bg-indigo-100 selection:text-indigo-900 overflow-hidden">
            <a
                href="#main-content"
                className="sr-only focus:not-sr-only focus:absolute focus:top-3 focus:left-3 focus:z-50 focus:px-3 focus:py-2 focus:bg-white focus:text-slate-900 focus:rounded-md focus:shadow focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500 focus-visible:ring-offset-2"
            >
                メインコンテンツへスキップ
            </a>
            ...
            <header className="bg-white/80 backdrop-blur-md border-b border-slate-200/60 px-8 py-5 flex flex-col xl:flex-row xl:items-center justify-between gap-6 sticky top-0 z-30 shadow-sm">
                <div className="flex items-center gap-4">
                    <div className="bg-premium-gradient p-2.5 rounded-2xl shadow-lg shadow-indigo-100 rotate-3">
                        <TrendingUp className="text-white w-7 h-7 -rotate-3" aria-hidden="true" />
                    </div>
                    <div>
                ...
                <div className="flex flex-wrap items-center gap-6">
                    <div className="px-4 py-2 rounded-2xl border border-slate-200 bg-white shadow-sm">
                        ...
                        <p className={`text-xs font-bold ${policyEnabled ? 'text-emerald-600' : 'text-slate-500'}`}>
            ...
            <main id="main-content" className="flex-1 min-h-0 flex flex-row relative">
                <div className={`flex-1 min-h-0 overflow-y-auto p-8 bg-slate-50/30 scroll-smooth`}>
                    <div className="max-w-[1400px] mx-auto">
                        ...
                        <div className="grid grid-cols-1 gap-8 mt-10">
                            ...
                            <div className="bg-white p-8 rounded-[2rem] premium-shadow border border-slate-200/60 ring-1 ring-slate-100">
                                ...
                                <AssetChart results={results} assets={assets} />
                            </div>
                        </div>
