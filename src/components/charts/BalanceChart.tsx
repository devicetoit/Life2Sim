...
export const BalanceChart: React.FC<Props> = ({ results }) => {
    ...
    const expenseConfigs = [
        { key: 'tax', name: '税・社会保', color: '#d97706' },
        { key: 'insurance', name: '保険料', color: '#8b5cf6' },
        { key: 'repayment', name: '返済額', color: '#ef4444' },
        { key: 'education', name: '教育費', color: '#ec4899' },
        { key: 'futurePlan', name: '将来プラン', color: '#f59e0b' },
        { key: 'housing', name: '住居関連', color: '#3b82f6' },
        { key: 'specific', name: '固有支出', color: '#6b7280' },
        { key: 'living', name: '生活費', color: '#10b981' },
    ];
        ...
        <div className="h-96 w-full bg-white p-4 rounded-lg shadow">
            ...
            <ResponsiveContainer width="100%" height="100%">
                <ComposedChart data={data} margin={{ top: 10, right: 30, left: 0, bottom: 0 }}>
                    ...
                    <Line type="monotone" dataKey="income" name="収入(額面)" stroke="#ef4444" strokeWidth={3} dot={false} />
                    <Line type="monotone" dataKey="balance" name="収支差(可処分)" stroke="#000" strokeWidth={1} strokeDasharray="5 5" dot={false} />
                </ComposedChart>
            </ResponsiveContainer>
        </div>
    );
